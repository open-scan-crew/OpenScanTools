...
void PostRenderer::setConstantHDR(float opacity, bool substract, bool noFlash, float flashExposure, HighlightCompressionMode compressionMode, VkExtent2D screenSize, Color32 background, VkCommandBuffer _cmdBuffer)
{
    ...
    int toneMode = compressionMode == HighlightCompressionMode::Exponential ? 1 : 0;
    h_pfn->vkCmdPushConstants(_cmdBuffer, m_transparencyHDRPipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 36, 4, &toneMode);
}
