...
double computeAverageSeedDistance(const std::vector<glm::dvec3>& seeds, const glm::dvec3& axisDirection, const glm::dvec3& axisPoint)
{
    if (seeds.empty())
        return 0.0;
    double sum = 0.0;
    for (const glm::dvec3& seed : seeds)
        sum += pointToAxisDistance(seed, axisDirection, axisPoint);
    return sum / static_cast<double>(seeds.size());
}

bool isCandidateRadiusPlausible(const std::vector<glm::dvec3>& seeds, const glm::dvec3& axisDirection, const glm::dvec3& axisPoint, const double radius)
{
    if (radius <= 1e-9 || seeds.empty())
        return false;
    const double averageSeedDistance = computeAverageSeedDistance(seeds, axisDirection, axisPoint);
    if (averageSeedDistance <= 1e-9)
        return false;

    // Keep a broad band to avoid blocking valid fits while rejecting obvious outliers.
    // This protects against tiny/huge radii caused by nearby walls or equipment contamination.
    const double minRatio = 0.35;
    const double maxRatio = 2.40;
    const double ratio = radius / averageSeedDistance;
...
bool TlScanOverseer::filterOutliersAndWrite(tls::ScanGuid _scanGuid, const TransformationModule& _modelMat, const ClippingAssembly& _clippingAssembly, int kNeighbors, const OutlierStats& stats, double nSigma, double beta, IScanFileWriter* _outScan, uint64_t& removedPoints, const ProgressCallback& progress, bool keepOutsidePoints)
{
    ...
    return scan->filterOutliersAndWrite(_modelMat, _clippingAssembly, kNeighbors, stats, nSigma, beta, _outScan, removedPoints, progress, keepOutsidePoints);
}
...
bool TlScanOverseer::balanceColorsAndWrite(tls::ScanGuid scanGuid, const TransformationModule& modelMat, const ClippingAssembly& clippingAssembly, int kMin, int kMax, double trimPercent, double sharpnessBlend, bool applyOnIntensity, bool applyOnRgb, const std::function<void(const GeometricBox&, std::vector<PointXYZIRGB>&)>& externalPointsProvider, IScanFileWriter* outScan, uint64_t& modifiedPoints, const ProgressCallback& progress, bool keepOutsidePoints)
{
    ...
    return scan->balanceColorsAndWrite(modelMat, clippingAssembly, kMin, kMax, trimPercent, sharpnessBlend, applyOnIntensity, applyOnRgb, externalPointsProvider, outScan, modifiedPoints, progress, keepOutsidePoints);
}
...
bool TlScanOverseer::fitCylinder(const glm::dvec3& globalSeedPoint, const double& radius, const double& threshold, double& cylinderRadius, glm::dvec3& cylinderDirection, glm::dvec3& cylinderCenter, const FitCylinderMode& mode, const ClippingAssembly& clippingAssembly)
{
    ...
    case::FitCylinderMode::robust:
    {
        ...
        searchRadiiToTry = { 0.15, 0.24, radius };
        break;
    }
    ...
    for (double currentSearchRadius : searchRadiiToTry)
    {
        ...
        if (!isCandidateRadiusPlausible(seedPoints, candidateDirection, candidateCenter, candidateRadius))
        {
            if (metricsEnabled)
            {
                Logger::log(LoggerMode::DataLog) << "[PIPE_METRICS] fitCylinder reject reason=radiusPlausibility candidateRadius="
                    << candidateRadius << Logger::endl;
            }
            continue;
        }
        ...
        if (!candidateFound || score < bestScore)
        {
            ...
            // Early stop: if compact search already gives a highly seed-consistent candidate,
            // skip larger neighborhoods to preserve performance in clean scenarios.
            if (score < 0.18 && currentSearchRadius <= 0.15)
                break;
        }
    }
    if (!candidateFound)
...
bool TlScanOverseer::fitCylinderMultipleSeeds(const std::vector<glm::dvec3>& globalSeedPoint, const double& radius, const double& threshold, double& cylinderRadius, glm::dvec3& cylinderDirection, glm::dvec3& cylinderCenter, const FitCylinderMode& mode, const ClippingAssembly& clippingAssembly)
{
    ...
    for (double currentSearchRadius : searchRadiiToTry)
    {
        ...
        if (!isCandidateRadiusPlausible(globalSeedPoint, candidateDirection, candidateCenter, candidateRadius))
        {
            if (metricsEnabled)
            {
                Logger::log(LoggerMode::DataLog) << "[PIPE_METRICS] fitCylinderMultipleSeeds reject reason=radiusPlausibility candidateRadius="
                    << candidateRadius << Logger::endl;
            }
            continue;
        }
        ...
        if (!candidateFound || score < bestScore)
        {
            ...
            // Early stop for seed-consistent candidates found with compact neighborhoods.
            if (score < 0.16 && currentSearchRadius <= 0.15)
                break;
        }
    }
    if (!candidateFound)
