...
bool TlScanOverseer::denoiseColorsAndWrite(tls::ScanGuid _scanGuid, const TransformationModule& _modelMat, const ClippingAssembly& _clippingAssembly, int kNeighbors, int strength, double radiusFactor, int iterations, bool preserveLuminance, IScanFileWriter* _outScan, uint64_t& processedPoints, const ProgressCallback& progress)
{
    ...
    return scan->denoiseColorsAndWrite(_modelMat, _clippingAssembly, kNeighbors, strength, radiusFactor, iterations, preserveLuminance, _outScan, processedPoints, progress);
}
