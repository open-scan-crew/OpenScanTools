...
{
    {
        // 2B.2 proactive pass: free one slot before opening a new TLS file for GUID resolution.
        std::lock_guard<std::mutex> lock(m_activeMutex);
...
void TlScanOverseer::evictScansIfNeeded_nolock(size_t reserveSlots)
{
    ...
    while (m_activeScans.size() + reserveSlots > m_maxActiveScans)
    {
        ...
        bool candidateIsSoftEviction = false;

        // First pass: normal eviction candidates (safe full deletion).
        ...
        // Second pass (2B.2): if nothing is fully evictable, perform a soft eviction
        // by releasing file handles early. This keeps memory objects alive until they
        // become deletable, while still reducing OS open-file pressure.
        ...
        if (candidateIsSoftEviction)
            scanToEvict->releaseFileHandle();
