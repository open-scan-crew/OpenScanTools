    ...
    SafePtr<CameraNode> cameraHD = m_graph.duplicateCamera(m_activeCamera);
    {
        ...
        auto updateOrthoMetadata = [&](const ProjectionFrustum& baseFrustum)
        {
            ...
            if (absDotZ >= verticalDotThreshold)
            {
                ...
                auto bottomLeftIt = corners.end();
                auto topRightIt = corners.end();
                for (auto it = corners.begin(); it != corners.end(); ++it)
                {
                    if (bottomLeftIt == corners.end() ||
                        it->screen.y > bottomLeftIt->screen.y ||
                        (it->screen.y == bottomLeftIt->screen.y && it->screen.x < bottomLeftIt->screen.x))
                    {
                        bottomLeftIt = it;
                    }

                    if (topRightIt == corners.end() ||
                        it->screen.y < topRightIt->screen.y ||
                        (it->screen.y == topRightIt->screen.y && it->screen.x > topRightIt->screen.x))
                    {
                        topRightIt = it;
                    }
                }
                ...
                if (bottomLeftIt != corners.end())
                {
                    ...
                    m_imageMetadata.bottomLeftXY = glm::dvec2(bottomLeftIt->world.x, topRightIt != corners.end() ? topRightIt->world.y : bottomLeftIt->world.y);
                }
                if (topRightIt != corners.end())
                {
                    m_imageMetadata.hasTopRightXY = true;
                    m_imageMetadata.topRightXY = glm::dvec2(topRightIt->world.x, bottomLeftIt != corners.end() ? bottomLeftIt->world.y : topRightIt->world.y);
                }
            }
            else if (absDotZ <= horizontalZThreshold)
            {
                const std::vector<CornerInfo> corners = computeCorners(baseFrustum);
                auto bottomIt = corners.end();
                for (auto it = corners.begin(); it != corners.end(); ++it)
                {
                    if (bottomIt == corners.end() ||
                        it->screen.y > bottomIt->screen.y)
                    {
                        bottomIt = it;
                    }
                }
