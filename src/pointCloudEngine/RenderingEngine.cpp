...
void RenderingEngine::updateHD()
{
    ...
    const double channelMax = bytePerPixel == 8u ? 65535.0 : 255.0;
    std::vector<double> accumulation(pixelCount * 3ull, 0.0);
    ...
    uint32_t completedPasses = 0;
    {
        ...
        for (uint32_t pass = 0; pass < aaPassCount; ++pass)
        {
            ...
            if (bytePerPixel == 8u)
            {
                ...
                {
                    const size_t base = pixel * 4ull;
                    const double r = static_cast<double>(src[base + 0]) / channelMax;
                    const double g = static_cast<double>(src[base + 1]) / channelMax;
                    const double b = static_cast<double>(src[base + 2]) / channelMax;
                    accumulation[pixel * 3ull + 0] += srgbToLinear(r);
                    accumulation[pixel * 3ull + 1] += srgbToLinear(g);
                    accumulation[pixel * 3ull + 2] += srgbToLinear(b);
                }
            }
            else
            {
    ...
    if (!accumulation.empty())
    {
        ...
        if (bytePerPixel == 8u)
        {
            ...
            for (size_t pixel = 0; pixel < pixelCount; ++pixel)
            {
                const size_t baseDst = pixel * 4ull;
                const double r = accumulation[pixel * 3ull + 0] * invPassCount;
                const double g = accumulation[pixel * 3ull + 1] * invPassCount;
                const double b = accumulation[pixel * 3ull + 2] * invPassCount;
                dst[baseDst + 0] = static_cast<uint16_t>(std::lround(std::clamp(linearToSrgb(r), 0.0, 1.0) * maxValue));
                dst[baseDst + 1] = static_cast<uint16_t>(std::lround(std::clamp(linearToSrgb(g), 0.0, 1.0) * maxValue));
                dst[baseDst + 2] = static_cast<uint16_t>(std::lround(std::clamp(linearToSrgb(b), 0.0, 1.0) * maxValue));
                dst[baseDst + 3] = std::numeric_limits<uint16_t>::max();
            }
        }
        else
        {
            ...
            for (size_t pixel = 0; pixel < pixelCount; ++pixel)
            {
                ...
                const double g = accumulation[pixel * 3ull + 1] * invPassCount;
                const double b = accumulation[pixel * 3ull + 2] * invPassCount;
                dst[baseDst + 0] = static_cast<uint8_t>(std::lround(std::clamp(linearToSrgb(r), 0.0, 1.0) * maxValue));
                dst[baseDst + 1] = static_cast<uint8_t>(std::lround(std::clamp(linearToSrgb(g), 0.0, 1.0) * maxValue));
                dst[baseDst + 2] = static_cast<uint8_t>(std::lround(std::clamp(linearToSrgb(b), 0.0, 1.0) * maxValue));
                dst[baseDst + 3] = std::numeric_limits<uint8_t>::max();
            }
        }
    }
