...
uint64_t HashFrame::hashRenderingData(VkExtent2D viewportExtent, const glm::dmat4& VP, const ClippingAssembly& clipAssembly, const std::vector<PointCloudDrawData>& m_pcDrawData, const DisplayParameters& display)
{
    ...
    hash += hash_fn_b(display.m_postRenderingSSAO.enabled);
    hash += hash_fn_f(display.m_postRenderingSSAO.size);
    hash += hash_fn_f(display.m_postRenderingSSAO.intensity);
    hash += hash_fn_f(display.m_postRenderingSSAO.bias);
    hash += hash_fn_f(display.m_postRenderingSSAO.minAO);
