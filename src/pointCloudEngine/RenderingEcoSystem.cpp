...
uint64_t HashFrame::hashRenderingData(VkExtent2D viewportExtent, const glm::dmat4& VP, const ClippingAssembly& clipAssembly, const std::vector<PointCloudDrawData>& m_pcDrawData, const DisplayParameters& display)
{
    ...
    hash += hash_fn_b(display.m_gapFillingGapOnly);
    hash += hash_fn_f(display.m_gapFillingGapDepthThreshold);
    hash += hash_fn_f(display.m_gapFillingFalloffStrength);
    hash += hash_fn_f(display.m_gapFillingFalloffExponent);
    hash += hash_fn_f(display.m_gapFillingDepthVarianceThreshold);
