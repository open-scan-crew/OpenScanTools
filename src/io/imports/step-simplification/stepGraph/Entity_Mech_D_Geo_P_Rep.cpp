...
Entity_Mech_D_Geo_P_Rep::Entity_Mech_D_Geo_P_Rep(unsigned int _id, const std::string& _name, const std::string& _arguments)
    : Entity(_id, _name, _arguments)
{}

bool Entity_Mech_D_Geo_P_Rep::canUpdateReference(Entity *entityChild, std::list<Entity*> &entitiesToDelete) {
    if (getNbParents() == 0 || getNbChildren() > 2) return true;
    return false;
}

void Entity_Mech_D_Geo_P_Rep::updateChildReference(Entity *entityChild, StepGraph& graph) {
    if (getNbChildren() == 2) graph.deleteEntity(*this);
    else (*this).updateReference(entityChild);
}
