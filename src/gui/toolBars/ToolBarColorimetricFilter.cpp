    ...
    , m_dataDispatcher(dataDispatcher)
{
    ...
    const int previewWidth = std::max(1, static_cast<int>(120 * guiScale));
    const int previewHeight = std::max(1, static_cast<int>(12 * guiScale));
    m_previewSize = QSize(previewWidth, previewHeight);
    m_ui.label_preview->setFixedSize(m_previewSize);
    m_ui.label_preview->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    Q_UNUSED(guiScale);
    updatePreview();
}
...
void ToolBarColorimetricFilter::onProjectLoad(IGuiData* data)
{
    ...
    if (!projectLoaded->m_isProjectLoad)
        m_focusCamera.reset();
}
...
void ToolBarColorimetricFilter::updateIntensityMode()
{
    ...
    {
        for (int i = 1; i < 4; ++i)
            updateColorField(i, glm::vec3(0.f), false);
    }
}

void ToolBarColorimetricFilter::updatePreview()
{
    int width = std::max(1, m_previewSize.width());
    int height = std::max(1, m_previewSize.height());
    ...
    {
        image.fill(toPreviewColor(m_colors[0]));
    }
    else if (count == 2)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            QColor qcolor = toPreviewColor(color);
        ...
        for (int y = 0; y < height; ++y)
        {
            for (int x = 0; x < width; ++x)
            {
                ...
                if (w1 >= 0.f && w2 >= 0.f && w3 >= 0.f)
                {
                    ...
                    image.setPixelColor(x, y, toPreviewColor(color));
                }
            }
        }
    }
...
void ToolBarColorimetricFilter::applyPickedColor(const Color32& color)
{
    ...
    glm::vec3 value = rgb;
    if (isIntensityMode(m_renderMode))
        value = glm::vec3(computeIntensity(color), 0.f, 0.f);
    else if (m_ui.combo_colorSpace->currentData().toInt() == static_cast<int>(ColorimetricFilterSpace::LAB))
        value = rgbToLab(rgb);
...
bool ToolBarColorimetricFilter::eventFilter(QObject* watched, QEvent* event)
{
    ...
    if (event->type() == QEvent::MouseButtonPress)
    {
        ...
        {
            viewport = qobject_cast<VulkanViewport*>(current);
            if (viewport)
                break;
        }
        if (viewport)
        {
            ...
            if (localPos.x() >= 0 && localPos.y() >= 0)
            {
                ...
                {
                    Color32 color = VulkanManager::getInstance().sampleColor(framebuffer,
                        static_cast<uint32_t>(localPos.x()),
                        static_cast<uint32_t>(localPos.y()));
...
void ToolBarColorimetricFilter::slotApply()
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        m_colors[i] = active ? value : glm::vec3(0.f);
...
void ToolBarColorimetricFilter::resetFilterState(bool resetTolerance)
{
    for (int i = 0; i < 4; ++i)
    {
...
float ToolBarColorimetricFilter::computeIntensity(const Color32& color) const
{
...
QColor ToolBarColorimetricFilter::toPreviewColor(const glm::vec3& value) const
{
    glm::vec3 rgb = value;
    if (isIntensityMode(m_renderMode))
    {
