    ...
    , m_focusCamera()
{
    ...
    m_ui.slider_billboardFeather->setValue(8);
    m_ui.doubleSpinBox_billboardFeather->setValue(8.0);
    m_ui.checkBox_adaptiveBillboard->setChecked(false);
    m_ui.slider_adaptiveStrength->setValue(50);
    m_ui.doubleSpinBox_adaptiveStrength->setValue(50.0);
    ...
    m_ui.checkBox_adaptiveBillboard->setEnabled(false);
    m_ui.slider_adaptiveStrength->setEnabled(false);
    m_ui.doubleSpinBox_adaptiveStrength->setEnabled(false);
    ...
    connect(m_ui.checkBox_adaptiveBillboard, &QCheckBox::stateChanged, this, &ToolBarRenderingAdvanced::slotAdaptiveBillboardToggled);
    connect(m_ui.slider_adaptiveStrength, &QSlider::valueChanged, this, &ToolBarRenderingAdvanced::slotAdaptiveStrengthSlider);
    connect(m_ui.doubleSpinBox_adaptiveStrength, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &ToolBarRenderingAdvanced::slotAdaptiveStrengthSpin);
...
void ToolBarRenderingAdvanced::updateBillboardUi(const BillboardRendering& billboardSettings)
{
    ...
    m_ui.checkBox_adaptiveBillboard->setChecked(billboardSettings.adaptiveSizeEnabled);
    double adaptivePercent = billboardSettings.adaptiveSizeStrength * 100.0;
    m_ui.slider_adaptiveStrength->setValue(static_cast<int>(std::round(adaptivePercent)));
    m_ui.doubleSpinBox_adaptiveStrength->setValue(adaptivePercent);
    ...
    m_ui.checkBox_adaptiveBillboard->setEnabled(enableControls);
    m_ui.slider_adaptiveStrength->setEnabled(enableControls && billboardSettings.adaptiveSizeEnabled);
    m_ui.doubleSpinBox_adaptiveStrength->setEnabled(enableControls && billboardSettings.adaptiveSizeEnabled);
}
...
void ToolBarRenderingAdvanced::blockAllSignals(bool block)
{
    ...
    m_ui.checkBox_adaptiveBillboard->blockSignals(block);
    m_ui.slider_adaptiveStrength->blockSignals(block);
    m_ui.doubleSpinBox_adaptiveStrength->blockSignals(block);
...
BillboardRendering ToolBarRenderingAdvanced::getBillboardFromUi() const
{
    ...
    settings.adaptiveSizeEnabled = m_ui.checkBox_adaptiveBillboard->isChecked();
    settings.adaptiveSizeStrength = static_cast<float>(m_ui.doubleSpinBox_adaptiveStrength->value() / 100.0);
    return settings;
}
...
void ToolBarRenderingAdvanced::slotBillboardToggled(int state)
{
    ...
    m_ui.checkBox_adaptiveBillboard->setEnabled(enabled);
    bool adaptiveEnabled = enabled && m_ui.checkBox_adaptiveBillboard->isChecked();
    m_ui.slider_adaptiveStrength->setEnabled(adaptiveEnabled);
    m_ui.doubleSpinBox_adaptiveStrength->setEnabled(adaptiveEnabled);
    sendBillboard();
}
...
void ToolBarRenderingAdvanced::slotAdaptiveBillboardToggled(int state)
{
    (void)state;
    bool adaptiveEnabled = m_ui.checkBox_billboardEnable->isChecked() && m_ui.checkBox_adaptiveBillboard->isChecked();
    m_ui.slider_adaptiveStrength->setEnabled(adaptiveEnabled);
    m_ui.doubleSpinBox_adaptiveStrength->setEnabled(adaptiveEnabled);
    if (m_ui.checkBox_billboardEnable->isChecked())
        sendBillboard();
...
void ToolBarRenderingAdvanced::slotAdaptiveStrengthSlider(int value)
{
    m_ui.doubleSpinBox_adaptiveStrength->setValue(value);
    if (m_ui.checkBox_billboardEnable->isChecked())
        sendBillboard();
}

void ToolBarRenderingAdvanced::slotAdaptiveStrengthSpin(double value)
{
    m_ui.slider_adaptiveStrength->setValue(static_cast<int>(std::round(value)));
    if (m_ui.checkBox_billboardEnable->isChecked())
        sendBillboard();
}
