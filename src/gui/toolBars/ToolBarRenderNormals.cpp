	...
	, m_focusCamera()
{
	...
	m_ui.checkBox_aoTopLight->setEnabled(true);
	...
	connect(m_ui.checkBox_aoTopLight, &QCheckBox::stateChanged, this, &ToolBarRenderNormals::slotAOChanged);
...
void ToolBarRenderNormals::updateAmbientOcclusion(const AmbientOcclusion& aoParams)
{
	...
	m_ui.checkBox_aoTopLight->setChecked(aoParams.useTopLight);
...
void ToolBarRenderNormals::blockAllSignals(bool block)
{
	...
	m_ui.checkBox_aoTopLight->blockSignals(block);
...
void ToolBarRenderNormals::slotAOChanged()
{
	...
	ao.useTopLight = m_ui.checkBox_aoTopLight->isChecked();
	ao.topLightStrength = 0.5f;
...
void ToolBarRenderNormals::applyAOEnabledState()
{
	...
	m_ui.checkBox_aoTopLight->setEnabled(aoControlsEnabled);
