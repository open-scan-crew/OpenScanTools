...
void DialogExportSubProject::startExport()
{
    ...
    ObjectStatusFilter filter = (ObjectStatusFilter)m_ui.objectsFilterComboBox->currentData().toInt();
    bool open_folder = m_ui.openExplorerFolderCheckBox->isChecked();

    m_dataDispatcher.sendControl(new control::io::ExportSubProject(m_exportFolder, m_storedProjectInfo, filter, open_folder));
    hide();
}
