...
ContextState ContextColorBalanceFilter::launch(Controller& controller)
{
    ...
    for (const SafePtr<PointCloudNode>& scan : scans)
    {
        ...
        bool res = TlScanOverseer::getInstance().balanceColorsAndWrite(balanceGuid, balanceTransform, *clippingToUse, m_kMin, m_kMax, m_trimPercent, m_sharpnessBlend, applyOnIntensity, applyOnRgb, externalProvider, scan_writer, modifiedPointCount, applyOnCurrentProject, progressCallback);
