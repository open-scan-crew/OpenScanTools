...
ContextState ContextExportStep::launch(Controller& controller)
{
    ...
    for (const SafePtr<AGraphNode>& exportObj : listToExport)
    {
        ...
        glm::dquat orientation;
        {
            ...
            size = readNode->getSize();
            orientation = readNode->getOrientation();
        }
        ...
        switch (type)
            ...
            {
                addPipingDirectory(id, controller);
                continue;
            }
            break;
            ...
            {
                primId = makeBox(size, position, orientation);
            }
            break;
...
void ContextExportStep::addTypeDirectory(ElementType type)
{
    ElementType toAdd;
    std::wstring name;
...
void ContextExportStep::getTypeNameDirectory(ElementType type, ElementType& directoryType, std::wstring& directoryName)
{
    directoryType = type;
    switch (type)
    {
