...
ContextState ContextColorBalance::launch(Controller& controller)
{
    ...
    std::unordered_map<tls::ScanGuid, tls::PointCloudInstance> scanInstancesByGuid;
    scanInstancesByGuid.reserve(scanInstances.size());
    for (const tls::PointCloudInstance& instance : scanInstances)
        scanInstancesByGuid.emplace(instance.header.guid, instance);

    std::vector<SafePtr<PointCloudNode>> activeScans;
    activeScans.reserve(scans.size());
    std::vector<tls::PointCloudInstance> workingInstances;
    workingInstances.reserve(scanInstances.size());
    std::unordered_map<tls::ScanGuid, BoundingBoxD> scanBboxes;
    scanBboxes.reserve(scanInstances.size());
    ...
    for (const SafePtr<PointCloudNode>& scan : scans)
    {
        ...
        {
            resolvedAssembly = clippingAssembly.resolveByPhase(rScan->getPhase());
            clippingToUse = &resolvedAssembly;
        }

        bool includeScan = true;
        if (!clippingToUse->empty())
            includeScan = TlScanOverseer::getInstance().testClippingEffect(rScan->getScanGuid(), rScan->getTransformationModule(), *clippingToUse);
        ...
        auto it_instance = scanInstancesByGuid.find(rScan->getScanGuid());
        tls::PointCloudInstance workingInstance;
        if (it_instance != scanInstancesByGuid.end())
        {
            workingInstance = it_instance->second;
        }
        else
        {
            tls::ScanHeader header;
            TlScanOverseer::getInstance().getScanHeader(rScan->getScanGuid(), header);
            header.name = rScan->getComposedName();
            workingInstance = tls::PointCloudInstance{ header, rScan->getTransformationModule(), rScan->getClippable(), rScan->getPhase() };
        }

        workingInstances.push_back(workingInstance);
        const tls::Limits& limits = workingInstance.header.limits;
        BoundingBoxD scan_bbox{ limits.xMin, limits.xMax,
                                limits.yMin, limits.yMax,
                                limits.zMin, limits.zMax };
        scanBboxes.emplace(workingInstance.header.guid, scan_bbox.transform(workingInstance.transfo.getTransformation()));
    }
    ...
    const uint64_t totalScans = activeScans.size();
    ...
    auto bboxesOverlap = [](const BoundingBoxD& lhs, const BoundingBoxD& rhs)
    {
        BoundingBoxD inter = lhs;
        inter.intersect(rhs);
        return inter.isValid();
    };

    for (const SafePtr<PointCloudNode>& scan : activeScans)
    {
        ...
        auto bboxIt = scanBboxes.find(old_guid);
        for (const tls::ScanGuid& guid : scanGuids)
        {
            if (guid == old_guid)
                continue;
            if (bboxIt != scanBboxes.end())
            {
                auto otherIt = scanBboxes.find(guid);
                if (otherIt != scanBboxes.end() && !bboxesOverlap(bboxIt->second, otherIt->second))
                    continue;
            }
