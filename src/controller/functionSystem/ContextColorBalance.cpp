...
namespace
{
    ...
    double computeSaturationRatio(const std::array<uint32_t, 256>& hist, uint64_t count, uint8_t threshold)
    {
        if (count == 0)
            return 0.0;
    ...
    void clampCorrection(ColorBalanceCellCorrection& correction, double minGain, double maxGain, double minOffset, double maxOffset)
    {
        if (correction.hasColor)
        {
            correction.gainL = std::clamp(correction.gainL, minGain, maxGain);
            correction.offsetL = std::clamp(correction.offsetL, minOffset, maxOffset);
        }
        if (correction.hasIntensity)
        {
            correction.gainI = std::clamp(correction.gainI, minGain, maxGain);
            correction.offsetI = std::clamp(correction.offsetI, minOffset, maxOffset);
        }
    }
...
ContextState ContextColorBalance::launch(Controller& controller)
{
    ...
    for (int level = 0; level < smoothingLevels; ++level)
    {
        ...
        constexpr double kLumaSaturationHigh = 235.0;
        constexpr double kLumaSigmaMin = 5.0;
        constexpr double kLumaSaturationRatioThreshold = 0.2;
        constexpr uint8_t kLumaSaturationBucket = 245;
        size_t flashRejectedCells = 0;
        ...
        for (size_t s = 0; s < scanList.size(); ++s)
        {
            ...
            for (const auto& entry : histMap)
            {
                ...
                if (scanUsage[s].useColor)
                {
                    double saturationRatio = computeSaturationRatio(entry.second.lumaHist, entry.second.count, kLumaSaturationBucket);
                    bool isFlashCell = (saturationRatio >= kLumaSaturationRatioThreshold) ||
                                       (cellStats.color.mean > kLumaSaturationHigh && cellStats.color.sigma < kLumaSigmaMin);
                    if (isFlashCell)
                    {
                        flashRejectedCells += 1;
                        continue;
                    }
                }
        ...
        constexpr double kMinGain = 0.6;
        constexpr double kMaxGain = 1.6;
        constexpr double kMinOffset = -40.0;
        constexpr double kMaxOffset = 40.0;
        for (auto& scanCorrections : corrections)
        {
            if (level < 0 || static_cast<size_t>(level) >= scanCorrections.size())
                continue;
            for (auto& entry : scanCorrections[level])
                clampCorrection(entry.second, kMinGain, kMaxGain, kMinOffset, kMaxOffset);
        }
        Logger::log(LoggerMode::FunctionLog)
            << "Color balance diagnostics (level " << level << "): overlap cells=" << overlapCells
            << ", valid cells=" << validCells << ", ratio=" << validRatio << Logger::endl;
        Logger::log(LoggerMode::FunctionLog)
            << "Color balance flash rejection (level " << level << "): rejected cells=" << flashRejectedCells << Logger::endl;
