...
IdGiver<uint32_t> AGraphNode::graphic_id_giver = IdGiver<uint32_t>(0);
...
AGraphNode::AGraphNode(const AGraphNode& node)
    ...
    , Data(node)
{
    graphic_id_ = graphic_id_giver.giveAutoId();
}
...
bool AGraphNode::isHovered() const
{
    return is_hovered_;
}

void AGraphNode::setHovered(bool hovered)
{
    is_hovered_ = hovered;
}
...
MeshDrawData AGraphNode::getMeshDrawData(const glm::dmat4& gTransfo) const
{
