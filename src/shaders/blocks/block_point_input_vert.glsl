...
{
    float trueTransparency = pc.transparency;
    ...
    if (pc.adaptiveTransparency != 0)
    {
        ...
        float uiTransparency = mix(uiNearFar.x, uiNearFar.y, k);
        trueTransparency = uiToTrueTransparency(clamp(uiTransparency, 1.0, 100.0));
    }
