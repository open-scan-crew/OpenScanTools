...
namespace temperature_scale
{
	...
	inline std::unordered_map<std::string, TemperatureScaleData>& cacheData()
	{
		static std::unordered_map<std::string, TemperatureScaleData> cache;
		return cache;
	}

	inline std::unordered_map<std::string, std::filesystem::file_time_type>& cacheTimes()
	{
		static std::unordered_map<std::string, std::filesystem::file_time_type> cache;
		return cache;
	}

	inline std::string makeCacheKey(const std::filesystem::path& path)
	{
		std::error_code ec;
		std::filesystem::path canonicalPath = std::filesystem::weakly_canonical(path, ec);
		if (ec)
			canonicalPath = path;
		return Utils::to_utf8(canonicalPath.wstring());
	}
	...
	{
		std::error_code ec;
		const std::string cacheKey = makeCacheKey(path);
		if (!std::filesystem::exists(path, ec))
		{
			cacheData().erase(cacheKey);
			cacheTimes().erase(cacheKey);
			if (errorMessage)
		...
		if (!ec)
		{
			auto it = cacheTimes().find(cacheKey);
			if (it != cacheTimes().end() && it->second == lastWrite)
			{
				outData = cacheData()[cacheKey];
				return true;
			}
		}
