...
private:
    ...
    static bool moreComplex(std::list<StepPiece*> v1, std::list<StepPiece*> v2);

public:
    ClassificationByComplexity(StepGraph* _stepGraph, unsigned int _nbClasses);
    ~ClassificationByComplexity();

    std::list<std::list<StepPiece*>> classify() override;
};

#endif // CLASSIFICATIONBYCOMPLEXITY_H
