...
private:
    const unsigned int nbClasses;
    static bool larger(std::list<StepPiece*> v1, std::list<StepPiece*> v2);

public:
    ClassificationByVolumeMBB(StepGraph *_stepGraph, unsigned int _nbClasses);
    ~ClassificationByVolumeMBB() override = default;

    std::list<std::list<StepPiece*>> classify() override;
    std::list<std::list<StepPiece *>> classifyOneVolumeOneClass(std::list<StepPiece *> stepPieces);
};

#endif // CLASSIFICATIONBYVOLUMEMBB_H
