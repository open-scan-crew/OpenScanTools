...
#ifndef QUICKCLASSIFICATIONBYSIMILARITY_H
#define QUICKCLASSIFICATIONBYSIMILARITY_H

#include <list>

#include "ClassificationStrategy.h"

class QuickClassificationBySimilarity : public ClassificationStrategy {

public:
    QuickClassificationBySimilarity(StepGraph* _stepGraph);
    ~QuickClassificationBySimilarity() = default;

    std::list<std::list<StepPiece*>> classify() override;
};

#endif // QUICKCLASSIFICATIONBYSIMILARITY_H
