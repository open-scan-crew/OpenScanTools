...
public:
    ...
    // Backward-compatible overload used by some callers/build configurations.
    // `preserveImageSettings=false` => full viewpoint restore (legacy behavior).
    // `preserveImageSettings=true`  => preserve current image toolbar settings.
    void snapToViewPoint(const SafePtr<ViewPointNode>& viewpoint, bool preserveImageSettings);
