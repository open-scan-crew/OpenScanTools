...
public:
    ...
    bool collectColorBalanceSamples(tls::ScanGuid scanGuid, const TransformationModule& modelMat, const ClippingAssembly& clippingAssembly, const glm::dvec3& origin, double voxelSize, size_t maxSamplesPerVoxel, std::unordered_map<int64_t, std::vector<PointXYZIRGB>>& samples, const ProgressCallback& progress = {});
    bool balanceColorsAndWrite(tls::ScanGuid scanGuid, const TransformationModule& modelMat, const ClippingAssembly& clippingAssembly, const ColorBalanceSettings& settings, IScanFileWriter* outScan, const ColorBalanceGlobalGrid* globalGrid, double globalRadius, const ProgressCallback& progress = {});
