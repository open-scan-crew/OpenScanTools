...
public:
    ...
    bool filterOutliersAndWrite(const TransformationModule& modelMat, const ClippingAssembly& clippingAssembly, int kNeighbors, const OutlierStats& stats, double nSigma, double beta, IScanFileWriter* writer, uint64_t& removedPoints, uint64_t* testedPoints = nullptr, uint64_t* keptPoints = nullptr, const ProgressCallback& progress = {});
