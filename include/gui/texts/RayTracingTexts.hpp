#ifndef RAY_TRACING_TXT_HPP
#define RAY_TRACING_TXT_HPP

#include <qstring.h>

#define TEXT_WAITING_CLICK_OR_TAG QObject::tr("Select a existing tag or pick a new position.")
#define TEXT_RAYTRACING_DEFAULT_1 QObject::tr("Pick the first point")
#define TEXT_RAYTRACING_DEFAULT_2 QObject::tr("Pick the second point")
#define TEXT_RAYTRACING_DEFAULT_3 QObject::tr("Pick the thrid point")
#define TEXT_RAYTRACING_FAILED QObject::tr("Ray failed to hit")

#endif